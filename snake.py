...
class button(object):
    def __init__(self,x=100, y=100,width=100, height=50)-> None:      
        self.x = x
        self.y = y
    ...
    def draw(self, surface):  
        ...
        pygame.draw.rect(surface, (0,255,0), self.button_rect)
        surface.blit(text, (self.x + 10, self.y + 10)) 
    


class cube(object):
    def __init__(self,start,dirnx=0, dirny=0,color=(255,0,0)) -> None:
        self.pos = start
        self.dirnx = dirnx
        self.dirny = dirny
        self.color = color
    ...
    def move(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if btn.button_rect.collidepoint(mouse_pos):
                    if btn.text == "Pause":
                        btn.text = "Resume"
...
def randomSnack (rows,mysnake):
    snakebody = mysnake.body
    while True:
        x = random.randrange(rows)
        y = random.randrange(rows)
        if len(list(filter(lambda z:z.pos == (x,y),snakebody))) > 0:
            continue
        else:
            break
    return (x,y)
...
def main():
    global width, rows,s,snack,ispause,btn
    width = 500
    rows = 20
    pygame.init()    
    pygame.font.init()
